"""Graph-event translation and response payloads for the web chat protocol."""
...
def chunk_text(chunk: Any) -> str:
    """Concatenate streamed text blocks; final messages use message_text instead."""
...
async def stream_chat_events(
    ...
    """Translate one graph turn to SSE and commit history only on completion."""
    ...
    try:
        ...
        yield sse(
            'final', {**final_payload(final_messages, turn_start_index), 'replace': emitted_text}
        )
        yield sse('done', {})
