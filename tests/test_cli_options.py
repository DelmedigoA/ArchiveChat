...
def isolated_environment(monkeypatch):
    ...
    for key in os.environ:
        if key.startswith('ARCHIVELENS_'):
...
def test_cli_overrides_yaml_which_overrides_environment(tmp_path, monkeypatch, web):
    ...
    monkeypatch.setenv('ARCHIVELENS_MODEL', 'env-model')
    monkeypatch.setenv('ARCHIVELENS_SEARCH_LIMIT', '8')
    monkeypatch.setenv('ARCHIVELENS_REASONING_EFFORT', 'medium')
    monkeypatch.setattr('sys.argv', ['archivelens', '--config', str(config), '--model', 'cli-model'])
...
def test_web_retains_pdf_and_port_defaults(monkeypatch):
    from archivelens.web.app import DOCUMENT_PDF_PATH, DOCUMENT_PDF_ROUTE

    monkeypatch.setattr('sys.argv', ['archivelens', '--port', '0'])
...
def test_missing_api_key_is_rejected_before_runtime_setup(monkeypatch, capsys, web):
    ...
    monkeypatch.setattr('sys.argv', ['archivelens'])
